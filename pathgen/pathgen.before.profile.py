...
def bias_drift(corr_time, drift, n, fs):
    ...
    for i in range(0, 3):
        if not math.isinf(corr_time[i]):
            ...
            for i in range(1, n):
                sensor_bias_drift[i, :] = a*sensor_bias_drift[i-1, :] + b*drift_noise[i-1, :]
        else:
            # normal distribution
...
def odo_gen(ref_odo, odo_err):
    ...
    Args:
        ref_odo: true odometer data.
        odo_err: odometer error profile.
            'scale': scale factor error.
            'std': RMS velocity error.
    Returns:
        odo_mea: measured odometer output.
    '''
    n = ref_odo.shape[0]
    scale_factor = odo_err['scale']
    odo_err = scale_factor*ref_odo + odo_err['std'] * np.random.randn(n, 3)
    return odo_err
    ...
    for i in range(0, motion_def.shape[0]):
        ...
        while (sim_count < sim_count_max) and (com_complete == 0):
            ...
            if (sim_count % sim_osr) == 0:
                ...
                # write to files
                imu_data[idx_high_freq, :] = np.hstack((round(sim_count / sim_osr),
                                                        acc_avg, gyro_avg))
                if ref_frame == 0:
                    nav_data[idx_high_freq, :] = np.hstack((round(sim_count / sim_osr),
                ...
                if magnet != 0:
                    ...
                    mag_data[idx_high_freq, :] = np.hstack((round(sim_count / sim_osr), geo_mag_b))
                # index increment
                idx_high_freq += 1
            ...
            if (sim_count % output_def[1, 1]) == 0:     # measurement period
                if output_def[1, 0] == 1:               # GPS
                    if ref_frame == 0:                  # NED frame
                        gps_data[idx_low_freq, :] = np.hstack((round(sim_count / sim_osr),
                                                               pos_n+pos_delta_n, vel_n))
                    else:                               # planar frame
                        gps_data[idx_low_freq, :] = np.hstack((round(sim_count / sim_osr),
                                                               pos_delta_n, vel_n))
                else:                                   # odometer
                    odo_data[idx_low_freq, :] = np.hstack((round(sim_count / sim_osr),
                                                           odo_dist, odo_vel))
                # index increment
                idx_low_freq += 1
...
def calc_true_sensor_output(pos_n, vel_b, att, c_nb, vel_dot_b, att_dot, ref_frame, g):
    ...
    # Velocity derivative
    vel_dot_n = c_nb.dot(vel_dot_b) + np.cross(w_nb_n, vel_n)
    # Position derivative
    ...
    acc = vel_dot_b + np.cross(w_ie_b+gyro, vel_b) + c_nb.T.dot(gravity)
